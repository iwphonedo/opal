...
class KafkaBroadcastSettings:
    ...
    def validate_dependencies(self):
        ...
        if not self.host:
            raise ValueError("KAFKA_HOST is required.")
        if not self.port:
            raise ValueError("KAFKA_PORT is required.")
    ...
    def load_from_env(self):
        ...
        self.port = int(os.getenv("KAFKA_PORT", 9093))
        ...
        self.kafka_port = os.getenv("KAFKA_CLIENT_PORT", 9093)
        ...
        self.kafka_ui_url = os.getenv(
            "KAFKA_UI_URL",
            f"http://{self.kafka_ui_container_name}:{self.kafka_ui_port}",
        )
